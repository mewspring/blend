...
func (blk *Block) ParseBody(order binary.ByteOrder, dna *DNA) (err error) {
	...
	if index == 0 {
		...
		case "AssetUUID":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*AssetUUID, blk.Hdr.Count)
				for i := range bodies {
					body := new(AssetUUID)
					err = binary.Read(r, order, body)
					if err != nil {
		...
		case "AssetUUIDList":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*AssetUUIDList, blk.Hdr.Count)
				for i := range bodies {
					body := new(AssetUUIDList)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
		...
		case "FSMenuEntry":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*FSMenuEntry, blk.Hdr.Count)
				for i := range bodies {
					body := new(FSMenuEntry)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
		...
		case "FileDirEntry":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*FileDirEntry, blk.Hdr.Count)
				for i := range bodies {
					body := new(FileDirEntry)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
			...
			/// ### [/ tmp ] ###
		case "FileDirEntryArr":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*FileDirEntryArr, blk.Hdr.Count)
				for i := range bodies {
					body := new(FileDirEntryArr)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
			...
			/// ### [/ tmp ] ###
		case "FileDirEntryRevision":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*FileDirEntryRevision, blk.Hdr.Count)
				for i := range bodies {
					body := new(FileDirEntryRevision)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
			...
			/// ### [/ tmp ] ###
		case "FileDirEntryVariant":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*FileDirEntryVariant, blk.Hdr.Count)
				for i := range bodies {
					body := new(FileDirEntryVariant)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
		...
		case "GPUFXSettings":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*GPUFXSettings, blk.Hdr.Count)
				for i := range bodies {
					body := new(GPUFXSettings)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
			...
			/// ### [/ tmp ] ###
		case "GPUSSAOSettings":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*GPUSSAOSettings, blk.Hdr.Count)
				for i := range bodies {
					body := new(GPUSSAOSettings)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
			...
			/// ### [/ tmp ] ###
		case "GP_BrushEdit_Settings":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*GP_BrushEdit_Settings, blk.Hdr.Count)
				for i := range bodies {
					body := new(GP_BrushEdit_Settings)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
			...
			/// ### [/ tmp ] ###
		case "GP_EditBrush_Data":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*GP_EditBrush_Data, blk.Hdr.Count)
				for i := range bodies {
					body := new(GP_EditBrush_Data)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
		...
		case "bMouseActuator":
			if blk.Hdr.Count > 1 {
				// Parse block body structures.
				bodies := make([]*BMouseActuator, blk.Hdr.Count)
				for i := range bodies {
					body := new(BMouseActuator)
					err = binary.Read(r, order, body)
					if err != nil {
						return err
					}
